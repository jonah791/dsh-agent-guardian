 ...
 *
 * 双胞胎声明：本文件是 `dsh-agent-sentinel/src/lease.ts` 的**同语义副本**（两个插件不跨包
 * 依赖——跨包 `file:` 依赖会在消费方 `.pnpm` 里产生陈旧副本，见 SOUL §5.15 §4）。改那边
 * 必须同步改这里，两边各有自己的单测（`tests/lease.test.mjs`）守护语义一致。
 * 本侧（guardian）用法：在「拉起前」与「唤醒前」查租约——被他方持有则收养而非拉起、不重复唤醒。
 */
